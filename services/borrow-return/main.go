...
func main() {
	...
	api := r.Group("/borrows")
	{
		api.POST("", CreateBorrow)            // ยืมหนังสือ
		api.POST("/:id/return", ReturnBorrow) // คืนหนังสือ
		api.GET("", GetBorrows)               // ดูประวัติการยืมทั้งหมด
		api.GET("/:id", GetBorrowByID)        // ดูประวัติการยืมราย ID
	}
...
// Get borrow by id
// ==========================================
func GetBorrowByID(c *gin.Context) {
	// param
	borrowID := c.Param("id")
